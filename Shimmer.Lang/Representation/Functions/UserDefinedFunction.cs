...
public class UserDefinedFunction(FunctionStmt declaration, Environment closure) : ShimmerFunction
{
    ...
    {
        var callEnvironment = BindParameters(args);
    ...
    private Environment BindParameters(IList<ShimmerValue> args)
    {
        if (Arity != args.Count)
